...
int vector_get(Vector *vector, int position) { return vector->items[position]; }
...
int main() {
  ...
  while ((nread = getline(&line, &len, stream)) != -1) {
    ...
    if (errno == ERANGE) {
      ...
      free_resources_part() return EXIT_FAILURE;
    }
  ...
  free_resources_part() return EXIT_SUCCESS;
}
